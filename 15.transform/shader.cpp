...
void Shader::setVec2(const QString &name, const QVector2D &vec2d)
{
    GLint location = shaderProgram.uniformLocation( name );
    shaderProgram.setUniformValue( location, vec2d );
}
...
void Shader::setMat4(const QString &name, const QMatrix4x4 &matrix4x4)
{
    GLint location = shaderProgram.uniformLocation( name );
    shaderProgram.setUniformValue( location, matrix4x4 );
}
